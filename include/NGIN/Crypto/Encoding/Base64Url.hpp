...
namespace NGIN::Crypto::Encoding
{
    ...
    [[nodiscard]] NGIN_CRYPTO_API CryptoExpected<std::string> EncodeBase64Url(ConstByteSpan input, Base64Padding padding = Base64Padding::Omit);

    /// @brief Encodes bytes as URL-safe Base64 text into caller-owned output.
    [[nodiscard]] NGIN_CRYPTO_API CryptoExpected<void> EncodeBase64UrlInto(
    ...
    [[nodiscard]] NGIN_CRYPTO_API CryptoExpected<ByteBuffer> DecodeBase64Url(std::string_view text);

    /// @brief Decodes strict URL-safe Base64 text into caller-owned output.
    [[nodiscard]] NGIN_CRYPTO_API CryptoExpected<void> DecodeBase64UrlInto(std::string_view text, ByteSpan output);
}// namespace NGIN::Crypto::Encoding
